...
## Часть 4: Обеспечение безопасности сервера

### Далее что можно сделать:

#### 1. Базовая защита сервера
- **SSH**: Отключить root-логин, использовать ключи аутентификация, ограничение по вводу не верных паролей
- **Firewall**: Настроить UFW\iptables, чтоб были открыты только необходимые порты
- **Обновления**: Регулярные обновления сервера и его пакетов, это прям желательно

#### 2. Защита сервисов
- **Docker**: Контейнеры запускать можно с ограничениями ресурсов
- **HAProxy**: Тут надо установить HTTPS и rate limiting желательно
- **Zabbix**: Можно поставить базовую авторизацию, либо использовать внутреннее ограничение по ip

#### 3. Мониторинг безопасности
- **Fail2ban**: Защита от brute-force атак на SSH
- **Логи**: Обезательно слать важные либо на другой сервер либо в оповищения
- **Аудиты**: Регулярные проверки логов
